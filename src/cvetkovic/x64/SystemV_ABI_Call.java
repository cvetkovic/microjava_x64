...
public class SystemV_ABI_Call {
    ...
    public List<String> generateCallForParameters(Obj function) {
        ...
        Iterator<Obj> iterator = function.getLocalSymbols().iterator();
        for (int i = 0; i < arguments.size(); i++) {
            RegisterDescriptor destination = getParameterRegister(i);
            Obj parameterVar = iterator.next();
            Obj parameterValue = arguments.get(i);

            // TODO: set addresses of parameters in symbol table
            if (destination != null) {
                resourceManager.fetchOperand(destination, parameterValue, instructions);
     ...
     */
    private RegisterDescriptor getParameterRegister(int orderNumber) {
        switch (orderNumber) {
            case 0:
